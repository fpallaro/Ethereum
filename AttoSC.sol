...
contract AttoSC { 
    ...
    function getStatoAdempimento(uint256 idAdempimento) public view returns (string) {
        ...
        if (keccak256(tipologia)==keccak256("Riscossione Polizze Vita")) {
            Adempimento_RISC_POLIZZA_VITA_SC adempimento_RISC_POLIZZA_VITA_SC= Adempimento_RISC_POLIZZA_VITA_SC(indirizzo);
            bytes32 tmp=adempimento_RISC_POLIZZA_VITA_SC.getStatus();
            rets=bytes32ToStr(tmp);
        }
        return rets;
    }  
